...
// Loop keeps going while the array is not a heap and the current element has at least a left child.
// If leftchild(index) is greater than the length of the array, the index does not have any children.

while(!isHeap && leftChild(index) < length){
    if(rightChild(index) >= length){        // no right child
        bigChildIndex = leftChild(index);
    }
    else if (arr[leftChild(index)] > arr[rightChild(index)]) {  // if left child is bigger of the two children
        bigChildIndex = leftChild(index);
    }
    else{   // Then right child is bigger
        bigChildIndex = rightChild(index);
    }
    // If the larger child's value is bigger than the current(parent) node, swap the values and continues the loop; otherwise it's a heap.
